...
INTERVAL is an optional string like \"2d\" or \"3h\"."
  (let* ((py org-reschedule-by-rule-python)
         (code (concat
                ...
                "            sys.stderr.write(\"Could not find reschedule date that satisfies constraints. Please reschedule manually.\")\n"
                "            sys.exit(1)\n"
                "        if candidate > now_dt and (cron_expr is None or croniter.croniter.match(cron_expr, candidate)):\n"
                "            next_time = candidate\n"
                "            break\n"
                "        candidate += delta\n"
                "elif cron_expr:\n"
                "    n = croniter.croniter(cron_expr, base)\n"
                "    next_time = n.get_next(datetime)\n"
                "    count = 0\n"
                "    while next_time <= now_dt:\n"
                "        count += 1\n"
                "        if count > MAX_ATTEMPTS:\n"
                "            sys.stderr.write(\"Could not find reschedule date that satisfies constraints. Please reschedule manually.\")\n"
                "            sys.exit(1)\n"
